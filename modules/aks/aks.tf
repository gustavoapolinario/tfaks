...
resource "azurerm_kubernetes_cluster" "aks" {
  ...
  default_node_pool {
    ...
    tags                 = merge(var.tags, { "pool" = "system" })
...
# Grant the AKS Cluster's Identity necessary permissions on the VNet/Subnet
# This is CRITICAL for the Azure CNI to function properly.
resource "azurerm_role_assignment" "network_contributor" {
  principal_id         = azurerm_kubernetes_cluster.aks.identity[0].principal_id
  role_definition_name = "Network Contributor"
  scope                = var.vnet_id
  # Avoid infinite loop if the identity isn't ready. Use depends_on or ignore_changes if needed.
